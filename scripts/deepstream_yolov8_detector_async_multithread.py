...
class SimpleDeepStreamPipeline:
    def __init__(self, input_uri, config_file):
        # Create the pipeline
        ...
        # TODO: this will receive the frames over the network, but we should change it to receive the frames from file directly


        # TODO: 
        # - deepstream fps rate limiter -> drop it in the second half of the pipeline
        # - make the model use int8
        # - check where we can improve our pipeline or the model further

        # Queue element
        self.queue = Gst.ElementFactory.make("queue", "queue")
        print("Queue element created.")

        # NVIDIA stream muxer to prepare for inference
        ...
        elements = [self.source_bin, self.queue, self.streammux, self.pgie, self.nvvidconv, 
                    self.caps1, self.nvvidconv2, self.caps2, self.appsink]
        for element in elements:
            if not element:
                sys.stderr.write(f"Unable to create element: {element}\n")
                sys.exit(1)
            self.pipeline.add(element)  # Ensure all elements are added to the pipeline
        ...
        self.pgie_src_pad = self.pgie.get_static_pad("src")
        if not self.pgie_src_pad:
            ...
            print("Added probe to extract detection metadata")
    ...
    def on_new_sample(self, sink):
        ...
        if result:
            ...
            with self.bbox_lock:
                bboxes = self.current_bboxes.copy() if self.current_bboxes else []
            
            
            # Put frame in queue for processing thread
            timestamp = time.time()
            try:
                self.frame_queue.put((frame_bgr.copy(), bboxes, timestamp), block=False)
            except queue.Full:
    ...
    def process_frames_thread(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                frame, bboxes, timestamp = self.frame_queue.get(timeout=0.1)
                ...
                # Render detection results including bounding boxes
                processed_frame = render_result(frame, bboxes, current_fps, avg_fps)
                
                # Print frame information to terminal periodically
                if self.total_frames % 30 == 0:
                    print(f"Frame #{self.total_frames}, Current FPS: {current_fps:.2f}, Average FPS: {avg_fps:.2f}, Detections: {len(bboxes)}")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("No input URI provided, using default.")
    ...
    pipeline = SimpleDeepStreamPipeline(input_uri, config_file)
